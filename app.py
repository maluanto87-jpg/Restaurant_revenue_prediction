...
with st.sidebar:
    st.header("Model artifacts model, scaler, encoders = get_artifacts()

model_name = type(model).__name__
needs_scaling = isinstance(model, SCALED_MODEL_TYPES)

# This line is only for checking which model is loaded
st.success(f"Loaded Model: {model_name}")

with st.sidebar:
    st.markdown(f"**Loaded model:** `{model_name}`")
    st.markdown(f"**Uses scaler at inference:** {'Yes' if needs_scaling else 'No'}")with st.sidebar:
    st.markdown(f"**Loaded model:** `{model_name}`")
    st.markdown(f"**Uses scaler at inference:** {'Yes' if needs_scaling else 'No'}")
